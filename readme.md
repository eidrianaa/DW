...
Mergi în meniul din stânga la iconița cu o săgeată în sus (Data Ingestion).
În căsuța Ticker Symbols, scrie: AAPL, MSFT, TSLA, GOOGL, NVDA.
Alege perioada 6 Months și apasă pe butonul Start Ingestion.
...
Mergi la prima iconiță din stânga (Home).
Numărul de TOTAL RECORDS nu mai este zero, ci va afișa volumul de date stocate, iar graficele de pe ecran vor prinde viață.
Pasul 3: Rulează calculele Big Data (Analytics)
Mergi la iconița de Analytics (graficele cu linii din meniu).
Apasă pe butonul roz Run Aggregation Job.
Ce se întâmplă: Containerul cu Apache Spark se va trezi instant, va citi datele brute din Cassandra, le va procesa prin algoritmi de map-reduce și va trimite rezultatele finale înapoi în interfață sub formă de grafice și tabele cu medii financiare.
